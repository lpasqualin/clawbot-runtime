import { E as isRecoverableTelegramNetworkError, b as withTelegramApiErrorLogging } from "./send-B5NZ9QZr.js";
import { n as readTelegramUpdateOffset, r as writeTelegramUpdateOffset } from "./update-offset-store-D3bph5cU.js";
import { t as createTelegramBot } from "./bot-gUR32RLX.js";
...
var TelegramPollingLivenessTracker = class {
	...
	detectStall(params) {
		if (!params.runnerIsRunning) return null;
...
const CONFIRM_PERSISTED_OFFSET_TIMEOUT_MS = 1e4;
...
const telegramApiTimeoutSignal = (timeoutMs) => AbortSignal.timeout(timeoutMs);
...
var TelegramPollingSession = class {
	...
	async #runPollingCycle(bot) {
		...
		try {
			...
			this.opts.log(`[telegram][diag] polling cycle error reason=${reason} ${liveness.formatDiagnosticFields("lastGetUpdatesError")} err=${errMsg}`);
