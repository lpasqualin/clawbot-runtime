import { a as resolveDefaultTelegramAccountId, o as resolveTelegramAccount, s as resolveTelegramMediaRuntimeOptions } from "./accounts-q2NkR7D9.js";
import { r as resolveTelegramTransport } from "./fetch-CQWpZY-X.js";
import { r as resolveTelegramInlineButtonsScope } from "./inline-buttons-B8GtAZuW.js";
import { f as shouldSuppressLocalTelegramExecApprovalPrompt, l as shouldEnableTelegramExecApprovalButtons, n as isTelegramExecApprovalApprover, r as isTelegramExecApprovalAuthorizedSender } from "./exec-approvals-DsCBiE53.js";
import { $ as getTelegramTextParts, A as tagTelegramNetworkError, D as isSafeToRetrySendError, E as isRecoverableTelegramNetworkError, F as buildTelegramRoutingTarget, H as resolveTelegramForumFlag, I as buildTelegramThreadParams, J as withResolvedTelegramForumFlag, K as resolveTelegramStreamMode, L as buildTypingThreadParams, M as buildTelegramGroupFrom, N as buildTelegramGroupPeerId, O as isTelegramClientRejection, P as buildTelegramParentPeer, Q as extractTelegramLocation, R as describeReplyTarget, U as resolveTelegramForumThreadId, V as resolveTelegramDirectPeerId, W as resolveTelegramGroupAllowFromContext, X as buildSenderName, Y as buildSenderLabel, Z as expandTextLinks, a as editMessageTelegram, at as firstDefined, b as withTelegramApiErrorLogging, ct as normalizeDmAllowFromWithStore, et as hasBotMention, g as recordSentMessage, it as resolveTelegramPrimaryMedia, j as buildGroupLabel, lt as resolveSenderAllowMatch, nt as normalizeForwardedContext, ot as isSenderAllowed, q as resolveTelegramThreadSpec, st as normalizeAllowFrom, v as buildInlineKeyboard, w as renderTelegramHtmlText, z as extractTelegramForumFlag } from "./send-B5NZ9QZr.js";
import { d as buildModelsKeyboard, g as resolveModelSelection, h as parseModelCallbackData, m as getModelsPageSize, o as buildTelegramModelsMenuButtons, p as calculateTotalPages, t as buildCommandsPaginationKeyboard } from "./command-ui-CyHJpkYk.js";
import { i as resolveTelegramCustomCommands, r as normalizeTelegramCommandName, t as TELEGRAM_COMMAND_NAME_PATTERN } from "./command-config-DA0b2Fs4.js";
import { t as createTelegramThreadBindingManager } from "./thread-bindings-B8QaQy7Y.js";
import { n as cacheSticker, t as describeStickerImage } from "./sticker-cache-DbKGqpw-.js";
import { n as evaluateTelegramGroupPolicyAccess, t as evaluateTelegramGroupBaseAccess } from "./group-access-CV6zsiTT.js";
import { a as buildCappedTelegramMenuCommands, i as createTelegramDraftStream, n as defaultTelegramBotDeps, o as buildPluginTelegramMenuCommands, r as resolveTelegramExecApproval, s as syncTelegramMenuCommands } from "./bot-deps-CWMurWhK.js";
import { n as deliverReplies, r as emitInternalMessageSentHook, t as resolveMedia } from "./delivery-DciAs6zG.js";
import { applyModelOverrideToSessionEntry, isNativeCommandsExplicitlyDisabled, loadConfig, loadSessionStore, loadSessionStore as loadSessionStore$1, resolveChannelContextVisibilityMode, resolveChannelGroupPolicy, resolveChannelGroupRequireMention, resolveMarkdownTableMode as resolveMarkdownTableMode$1, resolveNativeCommandsEnabled, resolveNativeSkillsEnabled, resolveSessionStoreEntry, resolveSessionStoreEntry as resolveSessionStoreEntry$1, updateSessionStore, writeConfigFile } from "openclaw/plugin-sdk/config-runtime";
...
async function loadTelegramSendRuntime() {
	telegramSendRuntimePromise ??= import("./send-B5NZ9QZr.js").then((n) => n.p);
...
async function loadTelegramNativeCommandDeliveryRuntime() {
	telegramNativeCommandDeliveryRuntimePromise ??= import("./bot-native-commands.delivery.runtime-B19VLkCI.js");
...
async function loadTelegramNativeCommandRuntime() {
	telegramNativeCommandRuntimePromise ??= import("./bot-native-commands.runtime-freIRl70.js");
...
function loadStickerVisionRuntime() {
	stickerVisionRuntimePromise ??= import("./sticker-vision.runtime-CfyQjKmO.js");
	return stickerVisionRuntimePromise;
}
function loadMediaUnderstandingRuntime() {
	mediaUnderstandingRuntimePromise ??= import("./media-understanding.runtime-Dr2jGRlz.js");
...
async function loadTelegramMessageContextSessionRuntime(runtime) {
	...
	return {
		...await import("./bot-message-context.session.runtime-CtBhs5e_.js"),
		...runtime
	};
}
...
async function loadTelegramMessageContextRuntime() {
	telegramMessageContextRuntimePromise ??= import("./bot-message-context.runtime-B7qorb8h.js");
...
const buildTelegramMessageContext = async ({ primaryCtx, allMedia, replyMedia = [], storeAllowFrom, options, bot, cfg, account, historyLimit, groupHistories, dmPolicy, allowFrom, groupAllowFrom, ackReactionScope, logger, resolveGroupActivation, resolveGroupRequireMention, resolveTelegramGroupConfig, loadFreshConfig, runtime, sessionRuntime, upsertPairingRequest, sendChatActionHandler }) => {
	...
	const freshCfg = loadFreshConfig?.() ?? (runtime?.loadConfig ?? (await loadTelegramMessageContextRuntime()).loadConfig)();
...
const dispatchTelegramMessage = async ({ context, bot, cfg, runtime, replyToMode, streamMode, textLimit, telegramCfg, telegramDeps: injectedTelegramDeps, opts }) => {
	const telegramDeps = injectedTelegramDeps ?? (await import("./bot-deps-CWMurWhK.js").then((n) => n.t)).defaultTelegramBotDeps;
...
function createTelegramBotCore(opts) {
	...
	const persistAcceptedUpdateId = (updateId) => {
		...
		if (highestPersistedUpdateId !== null && updateId <= highestPersistedUpdateId) return;
		highestPersistedUpdateId = updateId;
		Promise.resolve().then(() => opts.updateOffset?.onUpdateId?.(updateId)).catch((err) => {
	...
	const shouldSkipUpdate = (ctx) => {
		...
		if (skipped) logSkippedUpdate(key);
		return skipped;
	};
	bot.use(async (ctx, next) => {
		...
		if (typeof updateId === "number") persistAcceptedUpdateId(updateId);
		try {
			await next();
			completed = true;
		} finally {
			if (updateKey) {
				...
				if (completed) recentUpdates.check(updateKey);
				pendingUpdateKeys.delete(updateKey);
			}
			if (typeof updateId === "number") if (completed) failedUpdateIds.delete(updateId);
			else failedUpdateIds.add(updateId);
		}
	});
